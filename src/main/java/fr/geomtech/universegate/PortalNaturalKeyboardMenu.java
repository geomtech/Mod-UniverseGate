...
public class PortalNaturalKeyboardMenu extends AbstractContainerMenu {
    ...
    private final PortalNaturalKeyboardBlockEntity keyboard;
    ...
    // constructeur CLIENT
    public PortalNaturalKeyboardMenu(int syncId, Inventory inv, BlockPos pos) {
        this(syncId, inv, getKeyboardAt(inv, pos), new SimpleContainerData(1));
    }

    // constructeur SERVEUR
    public PortalNaturalKeyboardMenu(int syncId, Inventory inv, PortalNaturalKeyboardBlockEntity keyboard) {
        this(syncId, inv, keyboard, new SimpleContainerData(1));
    }

    protected PortalNaturalKeyboardMenu(int syncId, Inventory inv, PortalNaturalKeyboardBlockEntity keyboard, ContainerData data) {
        super(ModMenuTypes.PORTAL_NATURAL_KEYBOARD, syncId);
    ...
    private static PortalNaturalKeyboardBlockEntity getKeyboardAt(Inventory inv, BlockPos pos) {
        var be = inv.player.level().getBlockEntity(pos);
        if (be instanceof PortalNaturalKeyboardBlockEntity kb) return kb;
        if (be == null) {
            throw new IllegalStateException("Portal natural keyboard block entity not found at " + pos);
        }
        throw new IllegalStateException("Expected PortalNaturalKeyboardBlockEntity at " + pos + " but found " + be.getClass().getName());
    }
